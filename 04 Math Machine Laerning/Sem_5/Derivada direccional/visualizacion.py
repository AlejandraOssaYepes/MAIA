...
def dibujarCN(xlims, ylims, f, n=50, title='', fig=None, 
              showscale=False, ngrid=None, layout=LAYOUT):
    """ 
    ...
    title: [str] titulo
    fig: [plotly.Figure] figura sobre la cual dibujar
    showscale: [bool] True para mostrar barra de valor para colores
    layout: [dict] Opciones de visualización 
    ngrid: [int] número de lineas de grilla
    ___________________________________
    Salida:
    fig : [plotly.Figure] Figura de curvas de nivel
    """
    # Grilla
    ...
    if ngrid is not None:
        fig = add_manual_grid(fig, xlims, ylims, n=ngrid, line_width=2)

    return fig
    

def dibujar3D(xlims, ylims, f, n=50, title='', cn=False, showscale=False, layout=LAYOUT):
    """ 
    ...
    n: [int] número de puntos a evaluar
    title: [str] titulo
    contours: [bool] dibuja contornos en parte superior e inferior
    showscale: [bool] muestra barra de valores para colores
    layout: [dict] Opciones de visualización 
    ___________________________________
    Salida:
...
def dibujar_evaluacion(z, x1, x2, dim=2, fig=None, layout=LAYOUT):
    """ 
    ...
    if dim == 2:
        ...
        if fig is None:
            ...
            fig = copy(fig)
            line_marker = dict(color='#101010')
            fig.add_scatter(x=df['x'], y=df['z']) 
    elif dim == 3:
        # Organiza tabla
...
def add_manual_grid(fig, xlims, ylims, n=10, line_width=2, marker_color='white' ):
    fig = copy(fig)
